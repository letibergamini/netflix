...
print(movies.groupby("type").size())
...
dir_null = (movies[movies["director"].isnull()][["title", "type", "country"]])
print(dir_null.groupby("type").size())
 
cast_null = (movies[movies["cast"].isnull()][["title", "type", "country", "release_year"]])
cast_null_movies = cast_null[cast_null["type"] == "Movie"]
print(cast_null_movies)
print(cast_null.groupby("type").size())

movies["decade"] = (movies["release_year"] // 10) * 10
print(movies[["decade", "release_year"]])
decades_by_type = movies.groupby(["decade", "type"]).size()
decade_pivot = decades_by_type.unstack(fill_value=0)
print(decade_pivot)
...
plt.pie(type_counts, labels = type_counts.index, autopct="%1.1f%%", startangle=90, colors=["#FFB6A3", "#FFF1A6"])
...
movies_clean["year_added"] = movies_clean["year_added"].astype(int) 
movies_clean["month_added"] = movies_clean["month_added"].astype(int)
...
print(movies[movies["type"] == "Movie"]["duration_minutes"].isnull().sum()) 
movies_duration_df = movies[movies["type"] == "Movie"][["type", "duration_minutes"]]
...
sns.histplot(data=movies, x="decade", hue="type", binwidth=10, shrink=0.8)
plt.title("Number of Movies and TV Shows by Decade")
plt.xlabel("Decade")
plt.ylabel("Count")
plt.xticks(rotation=45)
...
movies_director_clean["director_clean"] = movies_director_clean["director"].str.split(r"\s*,\s*") 
print(movies_director_clean[["director", "director_clean"]][:20])

directors_flat = movies_director_clean["director_clean"].explode()
directors_count = directors_flat.value_counts()
top_directors = directors_count[:20]

import matplotlib.pyplot as plt
from matplotlib.ticker import MaxNLocator

plt.figure(figsize=(18, 10)) 
top_directors.plot(kind="bar", color = "#1f77b4")
...
most_popular_directors = movies_director_clean["director_clean"].value_counts()[:10]
...
plt.figure(figsize=(12, 6)) 
top_actors.plot(kind="bar", color = "#1f77b4")
...
plt.tight_layout()
plt.xticks(rotation=60, ha = "right")
for index, value in enumerate(top_movie_genres):
    plt.text(index, value, str(value), ha = "center", va = "bottom", fontsize = 8)
plt.show()
...
plt.tight_layout()
plt.xticks(rotation=60, ha = "right")
for index, value in enumerate(top_tvshows_genres):
    plt.text(index, value, str(value), ha = "center", va = "bottom", fontsize = 8)
plt.show()
